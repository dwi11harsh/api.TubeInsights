...
async def fetch_channel_metadata(url: str, include_video_metadata: bool = False, limit: Optional[int] = None, crawler_state: Optional[Dict[str, Any]] = None) -> Union[ChannelMetadata, None]:
    playwright = None
    browser = None
    try:
        ...
        print(f'error occurred while fetching channel metadata from url[{url}]: {e}')
        if browser:
            await browser.close()
        if playwright:
            await playwright.stop()
